...
#NOTE  annotation function to draw bounding boxes and labels on the image
def annotate_image(image, final_output):
    if final_output == "human_not_detected":
        cprint("No objects detected in the image.", "yellow")
        return image
...
async def connect_to_server(server_uri, video_path, prompt, step=10):
    """Connect to the server and send every nth frame from a video."""
    try:
        if step <= 0:
        ...
        async with websockets.connect(server_uri) as websocket:
            ...
            cprint(f"Total frames in video: {total_frames}", "cyan")
            ...
            for frame_number in range(0, total_frames, step):
                ...
                # Encode the frame to Base64 #NOTE split to 4 areas
                # frame = splitter_img(frame) #BUG splitter turn off
                _, buffer = cv2.imencode('.jpg', frame)
                frame_base64 = base64.b64encode(buffer).decode('utf-8')
                logger.debug(f"Extracted frame {frame_number}, size: {len(frame_base64)} characters")
                ...
                if "error" in result:
                    ...
                    if not output_text == 'human_not_detected':
                        cprint(output_text, 'blue')
                        string_list = output_text[8:-3] 
                        cprint(f"{string_list}",'yellow')
                        final_output = ast.literal_eval(string_list)
                        print(final_output)
                        frame  = annotate_image(frame, final_output)
                        cv2.imwrite(f'{folder_name}/frame_{frame_number}_.jpg', frame)  # Save the frame for debugging
        
                    else:
                        
                        cprint("No human detected in the image.", "yellow")
                        final_output = "human_not_detected"
                        cv2.imwrite(f'{folder_name}/frame_{frame_number}_.jpg', frame)  # Save the frame for debugging
...
def main():
    ...
    parser.add_argument(
        ...
        type=int, 
        default=0,
        help='Frame number to extract from video (if input is a video)'
    )
    parser.add_argument(
        '--step', 
        type=int, 
    ...
    asyncio.run(connect_to_server(args.server, args.media, args.prompt, args.step))

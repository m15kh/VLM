...
def annotate_image(image, final_output):
    ...
    if isinstance(final_output, dict):
        ...
        cprint("Unexpected string in final_output.", "red")
        return image
    else:
        cprint(f"Unsupported type: {type(final_output)}", "red")
        return image

    for i, det in enumerate(detections):
        if 'bbox_2d' not in det or 'label' not in det:
            cprint(f"Skipping item {i}: missing keys.", "yellow")
            continue
    ...
    try:
        ...
        async with websockets.connect(server_uri) as websocket:
            ...
            if "error" in result:
                ...
                cprint('---------------------###!####------------------','red')
                #cleanup output json for ready to model 
                ...
                if not output_text == 'human_not_detected':
                    ...
                    cprint("finally--------------", 'yellow')
                    cprint(final_output,'red')
                    img_out  = annotate_image(cv2.imread(image_path), final_output)  
                    
                    cv2.imwrite('annotated_image.jpg', img_out)
                    cprint("Annotated image saved as 'annotated_image.jpg'", "green")
   
                else:
                    
                    cv2.imwrite('annotated_image.jpg', cv2.imread(image_path))

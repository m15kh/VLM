...
class QwenInferenceServer:
    ...
    async def process_request(self, image_data):  # Removed the `prompt` parameter
        logger.info(colored("Processing new request...------------------------------------------------------", "yellow"))
        request_id = f"req_{tm.time():.6f}"
        
        # Read the prompt from the configuration file
        prompt = config.get('prompt', "What's in this image?") #defult prompt
    ...
    try:
        ...
        async for message in websocket:
            try:
                ...
                logger.debug(f"Received data from {client_id}: {data.keys()}")
                if "image" in data:  # Removed "prompt" check
                    image_data = data["image"]  # Base64 encoded image
                    
                    img_size = len(image_data) / 1024
                    cprint(f"Received request from {client_id}", "cyan")
                    logger.info(f"Image size: {img_size:.2f} KB")
                    
                    # Process the request
                    cprint(f"Processing request from {client_id}...", "yellow")
                    result = await server.process_request(image_data)
